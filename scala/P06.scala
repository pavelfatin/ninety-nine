...
def f1[T](list: List[T]): Boolean = list match {
  case Nil => true
  case _ :: Nil  => true
...
// Zip
def f3[T](list: List[T]): Boolean = list.zip(list.reverse).forall{case (a, b) => a == b}

// Zip again
def f4[T](list: List[T]): Boolean = list.zip(list.reverse).forall(((_: T) == (_: T)).tupled)
